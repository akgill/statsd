...
var process_metrics = function (metrics, metric_toggles, flushInterval, ts, flushCallback) {
    ...
    for (key in timers) {
      ...
      if (timers[key].length > 0) {
        ...
        if (metric_toggles["std"]) current_timer_data["std"] = stddev;
        if (metric_toggles["upper"]) current_timer_data["upper"] = max;
        if (metric_toggles["lower"]) current_timer_data["lower"] = min;
        if (metric_toggles["count"]) current_timer_data["count"] = timer_counters[key];
        if (metric_toggles["count_ps"]) current_timer_data["count_ps"] = timer_counters[key] / (flushInterval / 1000);
        if (metric_toggles["sum"]) current_timer_data["sum"] = sum;
        if (metric_toggles["sum_squares"]) current_timer_data["sum_squares"] = sumSquares;
        if (metric_toggles["mean"]) current_timer_data["mean"] = mean;
        if (metric_toggles["median"]) current_timer_data["median"] = median;
