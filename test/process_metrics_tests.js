...
module.exports = {
  ...
  counters_has_stats_count: function(test) {
    ...
    pm.process_metrics(this.metrics, this.metricToggles, 1000, this.time_stamp, function(){});
  ...
  counters_has_correct_rate: function(test) {
    test.expect(1);
    this.metrics.counters['a'] = 2;
    pm.process_metrics(this.metrics, this.metricToggles, 100, this.time_stamp, function(){});
  ...
  timers_single_time_toggle_off_single_metric: function(test) {
    test.expect(9);
    this.metrics.timers['a'] = [100];
    this.metrics.timer_counters['a'] = 1;
    ...
    test.equal(10, timer_data.count_ps);
    test.equal(100, timer_data.sum);
    test.equal(undefined, timer_data.sum_squares);
    test.equal(100, timer_data.mean);
    test.equal(100, timer_data.median);
    test.done();
  },
  timers_single_time_toggle_off_multiple_metrics: function(test) {
    test.expect(9);
    this.metrics.timers['a'] = [100];
    this.metrics.timer_counters['a'] = 1;
    ...
    timer_data = this.metrics.timer_data['a'];
    test.equal(0, timer_data.std);
    test.equal(undefined, timer_data.upper);
    test.equal(100, timer_data.lower);
    test.equal(1, timer_data.count);
    test.equal(undefined, timer_data.count_ps);
    test.equal(undefined, timer_data.sum);
    test.equal(undefined, timer_data.sum_squares);
    test.equal(100, timer_data.mean);
    test.equal(100, timer_data.median);
    test.done();
