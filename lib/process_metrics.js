...
var process_metrics = function (metrics, metricToggles, flushInterval, ts, flushCallback) {
    ...
    for (key in timers) {
      ...
      if (timers[key].length > 0) {
        ...
        if (metricToggles["std"]) current_timer_data["std"] = stddev;
        if (metricToggles["upper"]) current_timer_data["upper"] = max;
        if (metricToggles["lower"]) current_timer_data["lower"] = min;
        if (metricToggles["count"]) current_timer_data["count"] = timer_counters[key];
        if (metricToggles["count_ps"]) current_timer_data["count_ps"] = timer_counters[key] / (flushInterval / 1000);
        if (metricToggles["sum"]) current_timer_data["sum"] = sum;
        if (metricToggles["sum_squares"]) current_timer_data["sum_squares"] = sumSquares;
        if (metricToggles["mean"]) current_timer_data["mean"] = mean;
        if (metricToggles["median"]) current_timer_data["median"] = median;
